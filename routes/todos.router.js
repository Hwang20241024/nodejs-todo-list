...
router.post('/todos', async (req, res, next) => {
  try {
    ...
    const todo = new Todo({ value, order, doneAt: new Date() });
...
router.patch('/todos/:todoId', async (req, res) => {
  ...
  if (order) {
    ...
    if (targetTodo) {
      ...
      console.log(`현재 order 값: ${currentTodo.order}`); // 로그로 확인
  ...
  if (done !== undefined) {
    ...
    console.log(`현재 doneAt 값: ${currentTodo.doneAt}`); // 로그로 확인
  }

  if (value) {
    // 변경하려는 '해야할 일'의 내용을 변경합니다.
    currentTodo.value = value;
    console.log(`현재 해야할 일 값: ${currentTodo.value}`); // 로그로 확인
  }
